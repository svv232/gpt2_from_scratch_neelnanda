# -*- coding: utf-8 -*-
"""Copy of Clean_Transformer_Demo_Template.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1HFQcKEbxVRLFcVHYu2GZ2_po5ChoMqLv
...
import os
import transformers # Explicitly import transformers first
# Check if TRANSFORMERS_CACHE exists, if not, define it
if not hasattr(transformers, 'TRANSFORMERS_CACHE'):
    transformers.TRANSFORMERS_CACHE = os.path.expanduser("~/.cache/huggingface/transformers")
...
class LayerNorm(nn.Module):
    ...
    def forward(self, residual):
        ...
        x = residual - residual.mean(dim=-1, keepdim=True)
        x = x / (residual.var(dim=-1, keepdim=True, unbiased = False) + self.cfg.layer_norm_eps).sqrt()
...
class PosEmbed(nn.Module):
    ...
    def forward(self, tokens):
      batches, length = tokens.shape
      rows = torch.arange(length).unsqueeze(0)
      cols = torch.zeros(batches, dtype=int).unsqueeze(1)
      idx = rows + cols
...
import pysvelte
pysvelte.AttentionMulti(tokens=reference_gpt2.tokenizer.convert_ids_to_tokens(tokens[0]), attention=cache['blocks.0.attn.hook_attn'][0].permute(1, 2, 0)).show()
...
class Attention(nn.Module):
    ...
    def forward(self, normalized_resid_pre):
        ...
        num_heads, d_model, down_proj = self.W_Q.shape

        # compute query
        q = torch.einsum("bpd,ndj->bnpj", normalized_resid_pre, self.W_Q)
        q_bias = self.b_Q.unsqueeze(0).unsqueeze(2)
        q = q + q_bias

        # compute key
        k = torch.einsum("bpd,ndj->bnpj", normalized_resid_pre, self.W_K)
        k_bias = self.b_K.unsqueeze(0).unsqueeze(2)
        k = k + k_bias

        # attention mask
        attn_scores = q @ k.mT
        attn_scores = attn_scores / down_proj ** .5
        attention_mask = self.apply_causal_mask(attn_scores)

        attention_mask = attention_mask.softmax(dim=-1)

        # compute value
        v = torch.einsum("bpd,ndj->bnpj", normalized_resid_pre, self.W_V)
        v_bias = self.b_V.unsqueeze(0).unsqueeze(2)
        v = v + v_bias

        # compute output
        o = attention_mask @ v
        o = torch.einsum("bhsj,hjd->bsd", o, self.W_O)
        o = o + self.b_O
    ...
    def apply_causal_mask(self, attn_scores):
        ...
        mask_rows = torch.arange(query_pos).unsqueeze(0).cuda()
        mask_cols = torch.arange(key_pos).unsqueeze(1).cuda()
...
class MLP(nn.Module):
    ...
    def forward(self, normalized_resid_mid):
        ...
        mlp = normalized_resid_mid @ self.W_in
        mlp = mlp + self.b_in
        mlp = gelu_new(mlp)
        mlp = mlp @ self.W_out
        mlp = mlp + self.b_out
...
class TransformerBlock(nn.Module):
    ...
    def forward(self, resid_pre):
        ...
        resid_norm = self.ln1(resid_pre)
        attn_residual = self.attn(resid_norm)

        resid_pre = resid_pre + attn_residual

        resid_norm2 = self.ln2(resid_pre)
        mlp_residual = self.mlp(resid_norm2)

        return resid_pre + mlp_residual
...
class Unembed(nn.Module):
    ...
    def forward(self, normalized_resid_final):
        ...
        final = normalized_resid_final @ self.W_U
        final = final + self.b_U
        return final
...
class DemoTransformer(nn.Module):
    ...
    def forward(self, tokens):
        # tokens [batch, position]
        residual_stream = self.embed(tokens)
        positions = self.pos_embed(tokens)
        residual_stream = residual_stream + positions
...
# Mock torchvision.io.VideoReader to avoid ImportError
# This is a workaround for 'datasets' trying to import VideoReader even for text data
# and failing if the video backend for torchvision is not fully functional or configured.
if 'torchvision.io' in sys.modules:
    try:
        sys.modules['torchvision.io'].VideoReader = MockVideoReader
        print("Successfully mocked torchvision.io.VideoReader to MockVideoReader class.")
    except AttributeError:
        print("Could not set VideoReader to MockVideoReader on existing torchvision.io module.")
else:
    # If torchvision.io is not yet loaded, try to import it and then mock.
    # If the import itself fails, this block won't help directly, but the error
    # suggests torchvision.io is loadable, just VideoReader is not found within it.
    try:
        import torchvision.io
        torchvision.io.VideoReader = MockVideoReader
        print("Successfully imported and mocked torchvision.io.VideoReader to MockVideoReader class.")
    except (ImportError, AttributeError) as e:
        print(f"Could not import or mock torchvision.io.VideoReader: {e}")

# Alternatively, for datasets library specifically, one might use an environment variable
# like os.environ["HF_DATASETS_DISABLE_PLUGIN_LOADING"] = "1" for a broader workaround.
...
!pip uninstall -y torch torchvision torchaudio nvidia-cuda-runtime-cu13 nvidia-cudnn-cu13 nvidia-cublas-cu13
